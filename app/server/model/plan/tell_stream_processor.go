...
import (
	...
	"net/http"
	"llmcode-server/db"
	"llmcode-server/notify"
	"llmcode-server/types"
	"regexp"
	"runtime/debug"
	"strings"
	"time"
...
var openingTagRegex = regexp.MustCompile(`<LlmcodeBlock\s+lang="(.+?)"\s+path="(.+?)".*?>`)
...
func (state *activeTellStreamState) processChunk(choice types.ExtendedChatCompletionStreamChoice) processChunkResult {
	...
	if processor.fileOpen && strings.HasSuffix(active.CurrentReplyContent+content, "</LlmcodeBlock>") {
		if verboseLogging {
...
func (processor *chunkProcessor) bufferOrStream(content string, parserRes *types.ReplyParserRes, currentStage shared.CurrentStage, manualStopSequences []string) bufferOrStreamResult {
	...
	if awaitingTag {
		...
		if processor.awaitingBlockOpeningTag {
			...
			if parserRes.CurrentFilePath != "" {
				...
				if matched {
					...
					if verboseLogging {
						log.Printf("Opening <LlmcodeBlock> tag is missing even though parserRes.CurrentFile is set - something is wrong: %s\n", content)
					}
					processor.awaitingBlockOpeningTag = false
					processor.fileOpen = false
				...
				if len(split) > 1 {
					...
					if strings.HasPrefix(`LlmcodeBlock lang="`, last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(`LlmcodeBlock lang=", last)
						}
						shouldStream = false
						matchedPrefix = true
					} else if strings.HasPrefix(last, `LlmcodeBlock lang="`) {
						if verboseLogging {
			...
			if parserRes.CurrentFilePath == "" {
				if strings.Contains(content, "</LlmcodeBlock>") {
					shouldStream = true
					processor.awaitingBlockClosingTag = false
					processor.fileOpen = false
					// replace </LlmcodeBlock> with ``` to close the markdown code block
					content = strings.ReplaceAll(content, "</LlmcodeBlock>", "```")
				} else {
					log.Printf("Closing </LlmcodeBlock> tag is missing even though parserRes.CurrentOperation is nil - something is wrong: %s\n", content)
					processor.awaitingBlockClosingTag = false
					shouldStream = true
				}
			}
			...
			if strings.Contains(content, "<EndLlmcodeFileOps/>") {
				if verboseLogging {
					log.Printf("Found <EndLlmcodeFileOps/>\n")
				}
				processor.awaitingOpClosingTag = false
				content = strings.Replace(content, "\n<EndLlmcodeFileOps/>", "", 1)
				content = strings.Replace(content, "<EndLlmcodeFileOps/>", "", 1)
				shouldStream = true
			}
		}
		...
		if parserRes.MaybeFilePath != "" && parserRes.CurrentFilePath == "" {
			...
			// this will set processor.awaitingBlockOpeningTag to true if the content starts with any prefix of<LlmcodeBlock lang=" *or* any prefix of a full opening tag
		...
		if parserRes.CurrentFilePath != "" {
			...
			if strings.Contains(content, "</LlmcodeBlock>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"</LlmcodeBlock>\")")
				}
				processor.awaitingBlockClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"</LlmcodeBlock>\")")
				}
				split := strings.Split(content, "<")
				// log.Printf("split: %v\n", split)
				if len(split) > 1 {
					...
					last := split[len(split)-1]
					// log.Printf("last: %s\n", last)
					if strings.HasPrefix("/LlmcodeBlock>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"/LlmcodeBlock>\", last)")
						}
						processor.awaitingBlockClosingTag = true
					}
				}
			...
			if strings.Contains(content, "<EndLlmcodeFileOps/>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"<EndLlmcodeFileOps/>\")")
				}
				processor.awaitingOpClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"<EndLlmcodeFileOps/>\")")
				}
				split := strings.Split(content, "<")
				if len(split) > 1 {
					if verboseLogging {
					...
					last := split[len(split)-1]
					if strings.HasPrefix("EndLlmcodeFileOps/>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"EndLlmcodeFileOps/>\", last)")
						}
						processor.awaitingOpClosingTag = true
					}
				}
			}
		} else if strings.Contains(content, "</LlmcodeBlock>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"</LlmcodeBlock>\")")
			}
			content = strings.Replace(content, "</LlmcodeBlock>", "```", 1)
		} else if strings.Contains(content, "<EndLlmcodeFileOps/>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"<EndLlmcodeFileOps/>\")")
			}
			content = strings.Replace(content, "\n<EndLlmcodeFileOps/>", "", 1)
...
func replaceCodeBlockOpeningTag(content string, replaceWithFn func(lang string) string) (bool, string) {
	// check for opening tag matching <LlmcodeBlock lang="..." path="...">
	...
	if match != nil {
		...
		return true, strings.Replace(content, "<LlmcodeBlock>", replaceWithFn(""), 1)
	}

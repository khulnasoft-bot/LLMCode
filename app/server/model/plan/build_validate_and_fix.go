...
import (
	...
	"log"
	"math/rand"
	diff_pkg "llmcode-server/diff"
	"llmcode-server/model"
	"llmcode-server/model/prompts"
	"llmcode-server/syntax"
	"llmcode-server/types"
	"llmcode-server/utils"
...
func (fileState *activeBuildStreamFileState) buildValidate(
	...
	stop := []string{"<LlmcodeFinish/>"}
	if params.validateOnly {
		stop = []string{"<LlmcodeComments>", "<LlmcodeReplacements>"}
	}
...
func handleXMLResponse(
	...
	if strings.Contains(content, "<LlmcodeCorrect/>") {
	...
	replacementsOuter := utils.GetXMLContent(content, "LlmcodeReplacements")
	...
	if strings.Contains(content, "<LlmcodeIncorrect/>") {
		split := strings.Split(content, "<LlmcodeIncorrect/>")
		problem = split[0]
	} else if strings.Contains(content, "<LlmcodeReplacements>") {
		split := strings.Split(content, "<LlmcodeReplacements>")
		problem = split[0]
	}

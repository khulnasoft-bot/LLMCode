...
func TestBufferOrStream(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			initialState: &chunkProcessor{
				fileOpen:                true,
				contentBuffer:           `<LlmcodeBlock lang="go" path="main.go">` + "\n",
				awaitingBlockOpeningTag: true,
			},
			chunk:           `package`,
			maybeFilePath:   "",
		...
		},
		{
			...
			chunk:           `<LlmcodeBlock lang="go" path="main.go">` + "\npackage",
			maybeFilePath:   "",
			currentFilePath: "main.go",
			want: bufferOrStreamResult{
				shouldStream: true,
		...
		},
		{
			...
			wantState: &chunkProcessor{
				awaitingBlockOpeningTag: true,
				contentBuffer:           "something<LlmcodeBlock lang=\"go\" path=\"main",
			},
		},
		{
...

		{
			...
			chunk:        "hello <LlmcodeFinish/>bye",
			manualStop:   []string{"<LlmcodeFinish/>"},
			want: bufferOrStreamResult{
		...
		},
		{
			...
			chunk:      "<LlmcodeFin", // no '<' in second part
			manualStop: []string{"<LlmcodeFinish/>"},
			want: bufferOrStreamResult{
			...
			wantState: &chunkProcessor{
				contentBuffer: "<LlmcodeFin", // prefix is buffered
			},
		},
		{
			...
			initialState: &chunkProcessor{
				contentBuffer: "<LlmcodeFin", // leftover from previous call
			},

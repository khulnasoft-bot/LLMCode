...
If the changes were applied *correctly*, you MUST output a <LlmcodeCorrect/> tag, followed by a <LlmcodeFinish/> tag, then end your response, like this:
...
Next, you MUST output a <LlmcodeIncorrect/> tag, and then proceed to output the <LlmcodeComments/> tag and the <LlmcodeReplacements/> tag with at least one <Replacement> element (see below for details). Example:
...
Next, if the changes were applied *incorrectly*, you must analyze the *original file* and the *proposed updates* and output a <LlmcodeReplacements> element that applies the changes described in the *proposed updates* to the *original file* in order to produce a final, valid resulting file with all changes correctly applied.

CRITICALLY IMPORTANT: When applying changes with replacements, NO REFERENCE COMMENTS CAN BE PRESENT IN THE RESULTING FILE. All reference comments (as listed in the <LlmcodeComments> element above) ABSOLUTELY MUST be replaced with the code they refer to in the *original file*.

Now output a <LlmcodeReplacements> element that contains all the replacements needed to correctly apply the changes described in the *proposed updates* to the *original file*. The <LlmcodeReplacements> element MUST contain at least one <Replacement> element.
...
IMPORTANT RULES:
1. If your evaluation finds ANY issues, you MUST use <LlmcodeIncorrect/> followed by a <LlmcodeComments> element and a <LlmcodeReplacements> element with at least one <Replacement> element.
2. If your evaluation finds NO issues, you MUST use <LlmcodeCorrect/> then a <LlmcodeFinish/> element. Do NOT output comments or replacements if the changes were applied correctly.
...
5. Always include reasoning in a '## Evaluate Diff' section prior to outputting the <LlmcodeCorrect/> or <LlmcodeIncorrect/> tags.

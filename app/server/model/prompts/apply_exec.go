...
- _apply.sh:
<LlmcodeBlock lang="bash" path="_apply.sh">
...
- run.sh:
<LlmcodeBlock lang="bash" path="run.sh">
#!/bin/bash
set -euo pipefail
...
- Running web servers and browsers:
  ...
  * When starting a web server that needs a browser launched:
      ...
      * ALWAYS use the special command 'llmcode browser [urls...]' to launch the browser with one or more URLs. This command is provided by Llmcode and is available on all operating systems. Substitute the actual URL or URLs you want to open in place of [urls...]. This special command *blocks* and streams the browser output to the console. So if you need to run other commands *after* the browser is launched, you must background the browser command and correclty handle cleanup like other background processes. If the browser command exits with an error, kill any other background processes and exit the entire script with a non-zero exit code.
      ...
      - ALWAYS use 'llmcode browser' to open the browser and load urls. Do NOT use 'open' or 'xdg-open' or any other command to open the browser. USE 'llmcode browser' instead.
      * When using the 'llmcode browser' command, you ABSOLUTE MUST EXPLICITLY kill all other processes and exit the script with a non-zero exit code if the browser command fails. It is CRITICAL that you DO NOT omit this. The 'llmcode browser' command will fail if there are any uncaught errors or console.error logs in the browser.
      *CRUCIAL NOTE: the _apply.sh script will be run with 'set -e' (it will be set for you, don't add it yourself) so you must DIRECTLY handle errors in foreground commands and cleanup in a '|| { ... }' block immediately when the command fails. *This includes the 'llmcode browser' command.* Do NOT omit the '|| { ... }' block for 'llmcode browser' or any other foreground command.
...
- There must be NO lines between the file path and opening <LlmcodeBlock> tag
- Use lang="bash" in the <LlmcodeBlock> tag

When writing to _apply.sh include an ### Action Explanation Format section, a file path label, and a <LlmcodeBlock> tag that includes both a 'lang' attribute and a 'path' attribute as described in the instructions above.
...
- Llmcode automatically wraps ` + "`_apply.sh`" + ` in a script that enables job control and kills all processes if the user interrupts. Do NOT add ` + "`trap`" + `, ` + "`setsid`" + `, ` + "`nohup`" + `, or ` + "`disown`" + ` commands.
...
- Follow ALL instructions for *creating a new file* with an ### Action Explanation Format section, a file path label, and a <LlmcodeBlock> tag that includes both a 'lang' attribute and a 'path' attribute as described in the instructions above.
...
- Follow ALL instructions for *updating an existing file* with an ### Action Explanation Format section, a file path label, and a <LlmcodeBlock> tag that includes both a 'lang' attribute and a 'path' attribute as described in the instructions above.
...
- ALWAYS use <LlmcodeBlock lang="bash"> tags
...
Browser Commands:
- Use the special command 'llmcode browser [urls...]' to launch the browser with one or more URLs.
- This special command *blocks* and streams the browser output to the console.
- If commands are needed after launching browser with 'llmcode browser', background the browser command (handle cleanup like other background processes).
...
- ALWAYS use 'llmcode browser' to open the browser and load urls. Do NOT use 'open' or 'xdg-open' or any other command to open the browser. USE 'llmcode browser' instead.
- When using the 'llmcode browser' command, you ABSOLUTE MUST EXPLICITLY kill all other processes and exit the script with a non-zero exit code if the 'llmcode browser' command fails. It is CRITICAL that you DO NOT omit this. The 'llmcode browser' command will fail if there are any uncaught errors or console.error logs in the browser.
- CRUCIAL NOTE: the _apply.sh script will be run with 'set -e' (it will be set for you, don't add it yourself) so you must DIRECTLY handle errors in foreground commands and cleanup in a '|| { ... }' block immediately when the command fails. *This includes the 'llmcode browser' command.* Do NOT omit the '|| { ... }' block for 'llmcode browser' or any other foreground command.
Error Handling:
- Check for required tools

...
`
    ...
    `
      ...
      `
		...
		prompt += `
    This project is a NOT a git repository. When creating a new project from scratch, include a .llmcodeignore file in the root of the project.

    .llmcodeignore is a file that tells Llmcode which files and directories to ignore when loading context. Use it to prevent Llmcode from loading unnecessary, irrelevant, or sensitive files and directories.
    
    Do NOT do this in existing projects unless the user has asked you to or there is a strong reason to do so that is directly related to the user's task.

    If .llmcodeignore already exists in the project, consider whether there are any new files that should be added to it. If so, add a task to the plan to update the .llmcodeignore file accordingly.

    Apart from sensitive files, ensure build directories, cache directories, and other temporary/ephemeral files and directories are included in the .llmcodeignore file.
    `

		if params.ExecMode {
			prompt += `
      If you are writing any commands to the _apply.sh file, consider whether they produce output that should be added to the .llmcodeignore file. If so, add an additional task to the plan to update the .llmcodeignore file accordingly.
      `
		}
	}

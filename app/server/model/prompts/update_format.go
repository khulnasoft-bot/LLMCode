...
- example.js:
<LlmcodeBlock lang="javascript" path="example.js">
// ... existing code ...
...
- config.json:
<LlmcodeBlock lang="json" path="config.json">
{
  // ... existing code ...
...
- package.json:
<LlmcodeBlock lang="json" path="package.json">
{                                                                         
  "name": "vscode-llmcode",                                  
  "contributes": {                                                        
    "languages": [{                                                       
    ...
    "commands": [
      {
        "command": "llmcode.tellLlmcode",
      }
    ],
    "keybindings": [{
      "command": "llmcode.showFilePicker",
    }]
  },
  "scripts": {
    "compile": "webpack",
...
- array.js:
<LlmcodeBlock lang="javascript" path="array.js">
const a = [
  8,
  9,
  10,
...
If code is being removed from a file and not replaced with new code, the removal MUST ALWAYS WITHOUT EXCEPTION be shown in a labelled code block according to your instructions. Use the comment "// Llmcode: removed code" (with the appropriate comment symbol for the programming language) to denote the removal. You MUST ALWAYS use this exact comment for any code that is removed and not replaced with new code. DO NOT USE ANY OTHER COMMENT FOR CODE REMOVAL.

'// Llmcode: removed code' comments MUST *replace* the code that is being removed. The code that is being removed MUST NOT be included in the code block.
    
Do NOT use any other formatting apart from a labelled code block with the comment "// Llmcode: removed code" to denote code removal.
...
- main.js:
<LlmcodeBlock lang="javascript" path="main.js">
...
- src/main.go:
<LlmcodeBlock lang="go" path="src/main.go">
// ... existing code ...
...
- src/config.json:
<LlmcodeBlock lang="json" path="src/config.json">
{
  "database": {
    "host": "localhost",
    "port": 5432

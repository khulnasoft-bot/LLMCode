...
3. Output Context Sections
   ...
   - State "No context needs to be loaded." along with a brief conversational response and output <LlmcodeFinish/>
   ...
   c) Output <LlmcodeFinish/> immediately after

CRITICAL RULES:
- Do NOT write any code or implementation details
- Do NOT create tasks or plans
- Stop immediately after <LlmcodeFinish/>
...
`
  ...
  4. If you already have enough information from the project map to make a detailed plan or respond effectively to the user and so you won't need to load any additional context, then skip step 5 and output a <LlmcodeFinish/> immediately after steps 1 and 2 above.
  ...
  5. Otherwise, you MUST output:
     ...
     c) Immediately after the '### Files' list, output a <LlmcodeFinish/> tag. ***Do not output any text after <LlmcodeFinish/>.***
...
[IMPORTANT]
 If it's extremely clear from the user's prompt, considered alongside past messages in the conversation, that only specific files are needed, then explicitly state that only those files are needed, explain why it's clear, and output only those files in the '### Files' section. For example, if a user asks you to make a change to a specific file, and it's clear that no context beyond that file will be needed for the change, then state that only that file is needed based on the user's prompt, and then output *only* that file in the '### Files' section, then a <LlmcodeFinish/> tag. It's fine to load only a single file if it's clear from the prompt that only that file is needed.

- Immediately after the end of the '### Files' section list, you ABSOLUTELY MUST ALWAYS output a <LlmcodeFinish/> tag. You MUST NOT output any other text after the '### Files' section and you MUST NOT leave out the <LlmcodeFinish/> tag.

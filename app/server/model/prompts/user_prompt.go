...
At the end of the '### Tasks' section, you ABSOLUTELY MUST ALWAYS include a <LlmcodeFinish/> tag, then end the response.
...
After you have broken a task up in to multiple subtasks and output a '### Tasks' section, you *ABSOLUTELY MUST ALWAYS* output a <LlmcodeFinish/> tag and then end the response. You MUST ALWAYS output the <LlmcodeFinish/> tag at the end of the '### Tasks' section.

Output a <LlmcodeFinish/> tag after the '### Tasks' section. NEVER output a '### Tasks' section without also outputting a <LlmcodeFinish/> tag.
...
Always use an opening <LlmcodeBlock> tag to start a code block and a closing </LlmcodeBlock> tag to end a code block.

The <LlmcodeBlock> tag content MUST ONLY contain the code for the code block and NOTHING ELSE. Do NOT wrap the code block in triple backticks, CDATA tags, or any other text or formatting. Output ONLY the code and nothing else within the <LlmcodeBlock> tag.

The <LlmcodeBlock> tag MUST ALWAYS include both a 'lang' attribute and a 'path' attribute as described in the instructions above. It must not include any other attributes.
...
When *updating an existing file*, you MUST follow the instructions you've been given on how to update code in code blocks:
	...
	- Do NOT include line numbers in the <LlmcodeBlock> tag. While line numbers are included in the original file in context (prefixed with 'g4c-', like 'g4c-10: ') in context to assist you with describing the location of changes in the 'Action Explanation', they ABSOLUTELY MUST NOT be included in the <LlmcodeBlock> tag.
...
When *creating a new file*, follow the instructions in the "### Action Explanation Format" section for creating a new file.
  ...
  - The Summary field MUST briefly describe the new file and its purpose.
	...
	- The code itself MUST be written within a <LlmcodeBlock> tag.
	- The <LlmcodeBlock> tag MUST include both a 'lang' attribute and a 'path' attribute as described in the instructions above. It must not include any other attributes.
	- The <LlmcodeBlock> tag MUST NOT include any other text or formatting. It must only contain the code for the code block and NOTHING ELSE. Do NOT wrap the code block in triple backticks, CDATA tags, or any other text or formatting. Output ONLY the code and nothing else within the <LlmcodeBlock> tag.
...
2. Even for small changes:
   ...
   - No implementation and NO CODE until planning is complete, and you have output a '### Tasks' section and a <LlmcodeFinish/>
   - All changes must be in task list

3. The planning stage is *ALWAYS* required. You MUST NEVER skip ahead and start writing code in this response. You MUST complete the planning stage first and output a '### Tasks' section and a <LlmcodeFinish/> before you can start implementing code.
`
...
//    - You MUST output <LlmcodeFinish/> immediately after the task list
//    - You MUST end your response immediately after <LlmcodeFinish/>
...
//    C. "I need more context to continue. <LlmcodeFinish/>"
//    D. "I need more context to respond. <LlmcodeFinish/>"
//    E. "This is a significant update to the plan. I'll clear all context without pending changes, then decide what context I need to move forward. <LlmcodeFinish/>"
//    F. "This is a new task that is distinct from the plan. I'll clear all context without pending changes, then decide what context I need to move forward. <LlmcodeFinish/>"

// For statements A/B: You may rephrase naturally while keeping the meaning.
// For statements C/D: MUST include exact phrase "need more context" and <LlmcodeFinish/>.
// For statements E/F: MUST include exact phrase "clear all context" and <LlmcodeFinish/>.

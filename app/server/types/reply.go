...
func (r *ReplyParser) AddChunk(chunk string, addToTotal bool) {
	...
	if r.maybeFilePath != "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if strings.HasPrefix(prevFullLineTrimmed, "<LlmcodeBlock") {
			if verboseLogging {
	...
	if r.currentFilePath == "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if LineHasXmlPath(prevFullLineTrimmed) {
			if verboseLogging {
				log.Println("Line has XML-style LlmcodeBlock tag")
			}
		...
		if prevFullLineTrimmed == "</LlmcodeBlock>" {
			if verboseLogging {
		...
		if prevFullLineTrimmed == "<EndLlmcodeFileOps/>" {
			if verboseLogging {
...
func LineHasXmlPath(line string) bool {
	return strings.HasPrefix(line, "<LlmcodeBlock") && strings.Contains(line, `path="`)
}
...
func extractFilePath(line string) string {
	// Handle XML-style LlmcodeBlock tag
	if strings.HasPrefix(line, "<LlmcodeBlock") {

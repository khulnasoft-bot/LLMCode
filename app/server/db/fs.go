...
func init() {
	...
	log.Println("Llmcode server home dir:", home)
	log.Println("os.Getenv(LLMCODE_BASE_DIR):", os.Getenv("LLMCODE_BASE_DIR"))
	log.Println("GOENV:", os.Getenv("GOENV"))
	if os.Getenv("GOENV") == "development" && os.Getenv("LOCAL_MODE") == "1" {
		log.Println("Local mode enabled")
	}

	BaseDir = os.Getenv("LLMCODE_BASE_DIR")

	if BaseDir == "" {
		if os.Getenv("GOENV") == "development" {
			BaseDir = filepath.Join(home, "llmcode-server")
		} else {
			BaseDir = "/llmcode-server"
		}
	}

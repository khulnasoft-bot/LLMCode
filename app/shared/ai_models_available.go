...
When checking for sufficient credits on Llmcode Cloud, we use MaxOutputTokens-InputTokens, since this is the maximum that could hypothetically be used.
...
'ModelId' is the identifier for the model on the Llmcode side—it must be unique per provider. We have this so that models with the same name and provider, but different settings can be differentiated.

...
func init() {
	ReplSettingsDir = filepath.Join(fs.HomeLlmcodeDir, "repl_settings")
}
...
// ExecLlmcodeCommand spawns the same binary, wiring std streams directly so you
// don't have to capture output. Any os.Exit calls in the child won't kill your REPL.
func ExecLlmcodeCommand(args []string) (string, error) {
	return ExecLlmcodeCommandWithParams(args, ExecLlmcodeCommandParams{})
}

type ExecLlmcodeCommandParams struct {
	DisableSuggestions bool
	SessionId          string
}

func ExecLlmcodeCommandWithParams(args []string, params ExecLlmcodeCommandParams) (string, error) {
	// Create temp file
	tmpFile, err := os.CreateTemp("", "llmcode-output-*")
	if err != nil {
		return "", err
	}
	...
	if os.Getenv("LLMCODE_REPL") == "" {
		...
		env = append(env,
			"LLMCODE_REPL=1",
			"LLMCODE_REPL_OUTPUT_FILE="+tmpPath,
			"LLMCODE_COLUMNS="+strconv.Itoa(columns),
			"LLMCODE_STREAM_FOREGROUND_COLOR="+streamForegroundColor.Sequence(false),
			"GLAMOUR_STYLE="+glamourStyle,
			"LLMCODE_SKIP_UPGRADE=1",
		)

		if params.SessionId != "" {
			env = append(env, "LLMCODE_REPL_SESSION_ID="+params.SessionId)
		}
	}

	if params.DisableSuggestions {
		env = append(env, "LLMCODE_DISABLE_SUGGESTIONS=1")
	}

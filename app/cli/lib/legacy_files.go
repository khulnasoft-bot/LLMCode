...
func MigrateLegacyProjectFile(currentUserId string) {
	...
	// New formats map to a user id so we can handle multiple accounts in the same llmcode dir
	projectPath := filepath.Join(fs.LlmcodeDir, "project.json")
	if _, err := os.Stat(projectPath); err == nil {
		...
		v2Path := filepath.Join(fs.LlmcodeDir, "projects-v2.json")
...
func MigrateLegacyCurrentPlanFile(currentUserId string) {
	...
	planPath := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, "current_plan.json")
	...
	if _, err := os.Stat(planPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, "current-plans-v2.json")
...
func MigrateLegacyPlanSettingsFile(currentUserId string) {
	...
	settingsPath := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, CurrentPlanId, "settings.json")
	if _, err := os.Stat(settingsPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, CurrentPlanId, "settings-v2.json")

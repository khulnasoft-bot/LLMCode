...
func resolveProject(mustResolve, shouldCreate bool) {
	if fs.LlmcodeDir == "" {
		var err error
		if shouldCreate {
			_, _, err = fs.FindOrCreateLlmcode()
		} else {
			fs.FindLlmcodeDir()
		}

		if err != nil && mustResolve {
			term.OutputErrorAndExit("error finding or creating llmcode: %v", err)
		}
	}

	if (fs.LlmcodeDir == "" || fs.ProjectRoot == "") && mustResolve {
		fmt.Printf(
			"🤷‍♂️ No plans in current directory\nTry %s to create a plan or %s to see plans in nearby directories\n",
			color.New(color.Bold, term.ColorHiCyan).Sprint("llmcode new"),
			color.New(color.Bold, term.ColorHiCyan).Sprint("llmcode plans"))
		os.Exit(0)
	}
	...
	// check if projects-v2.json exists in LlmcodeDir
	path := filepath.Join(fs.LlmcodeDir, "projects-v2.json")
	_, err := os.Stat(path)
	...
	HomeCurrentProjectDir = filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId)
...
func mustInitProject(existingSettings *types.CurrentProjectSettingsByAccount) {
	...
	// write current-plans-v2.json to LlmcodeHomeDir/[projectId]/current-plans-v2.json
	dir := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId)

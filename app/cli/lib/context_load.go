...
func MustLoadContext(resources []string, params *types.LoadContextParams) {
	...
	if len(loadContextReq)+len(cachedMapPaths) == 0 {
		...
		if !didOutputReason {
			fmt.Println()
			fmt.Printf("Use %s to load a file or URL:", color.New(color.BgCyan, color.FgHiWhite).Sprint(" llmcode load [file-path|url] "))
			fmt.Println()
			fmt.Println("llmcode load file.c file.h")
			fmt.Println("llmcode load https://github.com/some-org/some-repo/README.md")

			fmt.Println()
			fmt.Printf("%s with the --recursive/-r flag:\n", color.New(color.Bold, term.ColorHiCyan).Sprint("Load a whole directory"))
			fmt.Println("llmcode load app/src -r")
...
func printIgnoredMsg() {
	fmt.Println()
	fmt.Println("ℹ️  " + color.New(color.FgWhite).Sprint("Due to .gitignore or .llmcodeignore, some paths weren't loaded.\nUse --force / -f to load ignored paths."))
}

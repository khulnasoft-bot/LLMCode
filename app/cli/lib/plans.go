...
func WriteCurrentPlan(id string) error {
	if fs.HomeLlmcodeDir == "" {
		return fmt.Errorf("HomeLlmcodeDir not set")
	}
...
func WriteCurrentBranch(branch string) error {
	...
	dir := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, CurrentPlanId)
...
func GetCurrentBranchNamesByPlanId(planIds []string) (map[string]string, error) {
	if fs.HomeLlmcodeDir == "" {
		return nil, fmt.Errorf("HomeLlmcodeDir not set")
	}
...
func getPlanCurrentBranch(planId string) (string, error) {
	...
	v2Path := filepath.Join(fs.HomeLlmcodeDir, CurrentProjectId, planId, "settings-v2.json")

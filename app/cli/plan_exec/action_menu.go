...
func handleHotkeyOption(option hotkeyOption, diffs []string, params ExecParams) {
	...
	if option.char == "d" {
		fmt.Println()
		_, err := lib.ExecLlmcodeCommandWithParams([]string{"diffs", "--ui", "--from-tell-menu"}, lib.ExecLlmcodeCommandParams{
		...
		fmt.Println()
		_, err := lib.ExecLlmcodeCommandWithParams([]string{"diffs", "--git"}, lib.ExecLlmcodeCommandParams{
			DisableSuggestions: true,
		})
		if err != nil {
			fmt.Printf("\nError showing diffs: %v\n", err)
		...
		fmt.Print("(a)")
		fmt.Println()
		_, err := lib.ExecLlmcodeCommand([]string{"apply"})
		if err != nil {
		...
		fmt.Println()
		_, err := lib.ExecLlmcodeCommand([]string{"reject"})
		if err != nil {
		...
		_, err := lib.ExecLlmcodeCommand([]string{"apply", "--full"})
		if err != nil {
			fmt.Printf("\nError applying changes: %v\n", err)
		}
		fmt.Println()

#!/usr/bin/env bash
#
# Llmcode Quick Installer
# This script installs the latest (or requested) version of Llmcode for supported platforms.
# See: https://docs.llmcode.khulnasoft.com
...
if [[ "${1:-}" =~ ^(-h|--help)$ ]]; then
  ...
  echo "Installs the latest (or requested) version of Llmcode for supported platforms."
  echo "Set LLMCODE_VERSION to install a specific version."
  exit 0
fi
...
ARCH=""
VERSION=""
RELEASES_URL="https://github.com/khulnasoft-lab/llmcode/releases/download"
...
INSTALL_TMPDIR="$(mktemp -d -t llmcode_install_XXXXXX)"
...
# Set Version
if [[ -z "${LLMCODE_VERSION}" ]]; then
  VERSION=$(curl -sL https://llmcode.khulnasoft.com/v2/cli-version.txt)
else
  VERSION=$LLMCODE_VERSION
...
welcome_llmcode () {
  ...
  echo ""
  echo "🚀 Llmcode v$VERSION • Quick Install"
  echo ""
  printf '%*s' "$(tput cols)" '' | tr ' ' -
  echo ""
}
...
download_llmcode () {
  ...
  url="${RELEASES_URL}/${ENCODED_TAG}/llmcode_${VERSION}_${PLATFORM}_${ARCH}.tar.gz"

  cd "$INSTALL_TMPDIR"

  echo "📥 Downloading Llmcode tarball"
  echo ""
  echo "👉 $url"
  echo ""
  curl -s -L -o llmcode.tar.gz "${url}"

  tar zxf llmcode.tar.gz 1> /dev/null

  should_sudo=false

  if [ "$PLATFORM" = "darwin" ] || $IS_DOCKER ; then
    if [[ -d /usr/local/bin ]]; then
      if ! mv llmcode /usr/local/bin/ 2>/dev/null; then
        echo "Permission denied when attempting to move Llmcode to /usr/local/bin."
        if command -v sudo 2>/dev/null; then
          should_sudo=true
          echo "Attempting to use sudo to complete installation."
          sudo mv llmcode /usr/local/bin/
          if sudo mv llmcode /usr/local/bin/; then
            echo "✅ Llmcode is installed in /usr/local/bin"
            echo ""
          else
            echo "Failed to install Llmcode using sudo. Please manually move Llmcode to a directory in your PATH."
            exit 1
          fi
        else
          echo "sudo not found. Please manually move Llmcode to a directory in your PATH."
          exit 1
        fi
      else
        echo "✅ Llmcode is installed in /usr/local/bin"
...
check_existing_installation () {
  if command -v llmcode >/dev/null 2>&1; then
    existing_version=$(llmcode version 2>/dev/null || echo "unknown")
    # Check if version starts with 1.x.x
    if [[ "$existing_version" =~ ^1\. ]]; then
      echo "Found existing Llmcode v1.x installation ($existing_version). Renaming to 'llmcode1' before installing v2..."
      
      # Get the location of existing binary
      existing_binary=$(command -v llmcode)
      binary_dir="$(dirname "$existing_binary")"
      
      # Rename llmcode to llmcode1
      if ! mv "$existing_binary" "${binary_dir}/llmcode1" 2>/dev/null; then
        if command -v sudo 1>/dev/null 2>&1; then
          sudo mv "$existing_binary" "${binary_dir}/llmcode1"
        else
          echo "Cannot rename old llmcode without sudo. Please rename manually."
        fi
      fi
      ...
      echo "Your v1.x installation is now accessible as 'llmcode1' and 'g4c1'"
    fi
  fi
}
...
printf '%*s' "$(tput cols)" '' | tr ' ' -
echo ""
echo "⚡️ Run 'llmcode' or 'g4c' in any project directory and start building!"
echo ""
printf '%*s' "$(tput cols)" '' | tr ' ' -
echo ""
echo "📚 Need help? 👉 https://docs.llmcode.khulnasoft.com"
echo ""

...
const (
	AuthTrialOption   = "Start a trial on Llmcode Cloud"
...
const (
	SignInCloudOption = "Llmcode Cloud"
	SignInLocalOption = "Local mode host"
	SignInOtherOption = "Another host"
)

func promptSignInNewAccount() error {
	selected, err := term.SelectFromList("Use Llmcode Cloud or another host?", []string{SignInCloudOption, SignInLocalOption, SignInOtherOption})
	...
	if selected == SignInCloudOption {
		...
		if selected == SignInLocalOption {
			email = "local-admin@llmcode.khulnasoft.com"
		} else {
			email, err = term.GetRequiredUserStringInput("Your email:")
		}

...
func checkForUpgrade() {
	if os.Getenv("LLMCODE_SKIP_UPGRADE") != "" {
		return
	}
	...
	defer cancel()
	latestVersionURL := "https://llmcode.khulnasoft.com/v2/cli-version.txt"
	...
	if latestVersion.GreaterThan(currentVersion) {
		term.StopSpinner()
		fmt.Println("A new version of Llmcode is available:", color.New(color.Bold, term.ColorHiGreen).Sprint(versionStr))
		...
		if confirmed {
			...
			fmt.Println("Note: set LLMCODE_SKIP_UPGRADE=1 to stop upgrade prompts")
		}
	}
}
...
func doUpgrade(version string) error {
	...
	downloadURL := fmt.Sprintf("https://github.com/khulnasoft-lab/llmcode/releases/download/%s/llmcode_%s_%s_%s.tar.gz", escapedTag, version, runtime.GOOS, runtime.GOARCH)
	...
	for {
		...
		if header.Typeflag == tar.TypeReg && (header.Name == "llmcode" || header.Name == "llmcode.exe") {

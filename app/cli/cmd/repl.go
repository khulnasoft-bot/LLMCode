...
var replCmd = &cobra.Command{
	Use:   "repl",
	Short: "Start interactive Llmcode REPL",
	Run:   runRepl,
}
...
func runRepl(cmd *cobra.Command, args []string) {
	...
	if lib.CurrentPlanId == "" {
		os.Setenv("LLMCODE_DISABLE_SUGGESTIONS", "1")
		args := []string{}
		...
		os.Setenv("LLMCODE_DISABLE_SUGGESTIONS", "")
		afterNew = true
	}
	...
	p = prompt.New(
		...
		prompt.WithTitle("Llmcode "+version.Version),
...
func executor(in string, p *prompt.Prompt) {
	...
	// Handle llmcode/g4c command prefix
	if strings.HasPrefix(lastLine, "llmcode ") || strings.HasPrefix(lastLine, "g4c ") {
		fmt.Println()
		parts := strings.Fields(lastLine)
		if len(parts) > 1 {
			args := parts[1:] // Skip the "llmcode" or "g4c" command
			_, err := lib.ExecLlmcodeCommandWithParams(args, lib.ExecLlmcodeCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
	...
	if lib.CurrentReplState.Mode == lib.ReplModeTell {
		...
		output, err := lib.ExecLlmcodeCommandWithParams(args, lib.ExecLlmcodeCommandParams{
			SessionId: sessionId,
		})
		if err != nil {
...
func replWelcome(params replWelcomeParams) {
	...
	color.New(color.FgHiWhite, color.BgBlue, color.Bold).Print(" 👋 Welcome to Llmcode ")
...
func handleRunCommand(args []string) error {
	...
	// Execute the command
	_, err := lib.ExecLlmcodeCommand(cmdArgs)
	if err != nil {
...
func execWithInput(params execWithInputParams) execWithInputResult {
	...
	default:
		...
		if matchedCmd != "" {
			// fmt.Println("> llmcode " + config.Cmd)
			if lastBackslashIndex > 0 {
				preservedBuffer += lastLine[:lastBackslashIndex]
			}
			fmt.Println()
			...
			_, err := lib.ExecLlmcodeCommandWithParams(execArgs, lib.ExecLlmcodeCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
				color.New(term.ColorHiRed).Printf("Error executing command: %v\n", err)

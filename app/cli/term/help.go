...
var CliCommands = []CmdConfig{
	{"", "", "start the Llmcode REPL", false},

	// {"--full", "", fmt.Sprintf("start the Llmcode REPL with auto-mode %s", "'full'"), false},
	// {"--semi", "", fmt.Sprintf("start the Llmcode REPL with auto-mode %s", "'semi'"), false},
	// {"--plus", "", fmt.Sprintf("start the Llmcode REPL with auto-mode %s", "'plus'"), false},
	// {"--basic", "", fmt.Sprintf("start the Llmcode REPL with auto-mode %s", "'basic'"), false},
	// {"--none", "", fmt.Sprintf("start the Llmcode REPL with auto-mode %s", "'none'"), false},

	// {"--daily", "", fmt.Sprintf("start the Llmcode REPL with %s model pack", "'daily-driver'"), false},
	// {"--strong", "", fmt.Sprintf("start the Llmcode REPL with %s model pack", "'strong'"), false},
	// {"--cheap", "", fmt.Sprintf("start the Llmcode REPL with %s model pack", "'cheap'"), false},
	// {"--oss", "", fmt.Sprintf("start the Llmcode REPL with %s model pack", "'oss'"), false},
	...
	{"usage", "", "show Llmcode Cloud current balance and usage report", true},
	{"usage --today", "", "show Llmcode Cloud usage for the day so far", true},
	{"usage --month", "", "show Llmcode Cloud usage for the current billing month", true},
	{"usage --plan", "", "show Llmcode Cloud usage for the current plan", true},

	{"usage --log", "", "show Llmcode Cloud transaction log", true},

	{"billing", "", "show Llmcode Cloud billing settings", true},
}
...
func printCmds(w io.Writer, prefix string, colors []color.Attribute, cmds ...string) {
	if os.Getenv("LLMCODE_DISABLE_SUGGESTIONS") != "" {
		return
	}
	...
	for _, cmd := range cmds {
		...
		if IsRepl {
			...
			styled = color.New(colors...).Sprintf(" llmcode ")
		} else {
			styled = color.New(colors...).Sprintf(" llmcode %s ", cmd)
		}
...
func PrintCustomCmd(prefix, cmd, alias, desc string) {
	cmd = strings.Replace(cmd, alias, fmt.Sprintf("(%s)", alias), 1)
	// desc += color.New(color.FgWhite).Sprintf(" • alias → %s", fmt.Sprint(a'lias'))
	styled := color.New(color.Bold, color.FgHiWhite, color.BgCyan, color.FgHiWhite).Sprintf(" llmcode %s ", cmd)
	fmt.Printf("%s%s 👉 %s\n", prefix, styled, desc)
}

// PrintCustomHelp prints the custom help output for the Llmcode CLI
func PrintCustomHelp(all bool) {
	builder := &strings.Builder{}

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Usage ")
	color.New(color.Bold).Fprintln(builder, "  llmcode [command] [flags]")
	...
	color.New(color.Bold).Fprintln(builder, "  llmcode help # show basic usage")
	color.New(color.Bold).Fprintln(builder, "  llmcode help --all # show all commands")
	color.New(color.Bold).Fprintln(builder, "  llmcode [command] --help")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgMagenta, color.FgHiWhite).Fprintln(builder, " Getting Started ")
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 🚀 Start the Llmcode REPL in a project directory with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" llmcode "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" g4c "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 💻 You can also use any command outside the REPL with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" llmcode [command] "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" g4c [command] "))
	fmt.Fprintln(builder)
	...
	if all {
		...
		color.New(color.Bold, color.BgHiBlue, color.FgHiWhite).Fprintln(builder, " Use 'llmcode help --all' or 'llmcode help -a' for a list of all commands ")
		fmt.Fprintln(builder)
